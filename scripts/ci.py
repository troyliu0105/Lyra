...
def prepare_game_sources(
    workspace: Path, downloaded_files: dict, prepare_dir: Path
...
def cmd_prepare_package(args):
    ...
    if polyfill_zip_dir and polyfill_zip_dir.exists():
        versions_to_process.append(
            (polyfill_zip_dir, True, downloaded_files.get("polyfill_zip"))
        )
...
def cmd_check_update(args):
    ...
    # 获取本仓库最新tag
    try:
        result = run_command(["git", "tag", "--sort=-v:refname"], capture_output=True)
...
def main():
    """主入口"""
    parser = argparse.ArgumentParser(description="GitHub Actions 辅助脚本")
    ...
    prep_parser.add_argument(
        "--tag", help="tag参数（格式如 v0.5.7.9-5.0.2a-0112，用于指定版本和日期）"
    )
    ...
    build_all_parallel_parser = subparsers.add_parser(
        "build-all-parallel", help="并行批量构建所有组合"
    )
    build_all_parallel_parser.add_argument(
        "pack_type",
        nargs="?",
        choices=["zip", "apk"],
        help="包类型（可选，默认构建两种）",
    ...
    build_all_parallel_parser.add_argument(
        "--jobs", "-j", type=int, help="并发进程数（默认为CPU核心数或4，取较小值）"
    )
    ...
    matrix_parser.add_argument(
        "--output-format", choices=["github", "json", "shell"], default="json"
    )
