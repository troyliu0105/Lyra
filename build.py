...
""",
    ...
    parser.add_argument(
        "pack_type", nargs="?", choices=["zip", "apk"], help="包类型: zip 或 apk"
    )

    parser.add_argument(
        "mod_code", nargs="?", help='MOD代码，可以是数字、"polyfill-数字" 或 "all"'
    )
    ...
    parser.add_argument(
        "-s", "--source", type=Path, default=None, help="源文件路径 (默认: 自动查找)"
    )

    parser.add_argument(
        "--list-combinations", action="store_true", help="列出所有可用的MOD组合"
    )

    parser.add_argument(
        "--generate-page", metavar="VERSION", help="生成指定版本的下载页面"
    )

    parser.add_argument(
        "--base-zip", type=Path, default=None, help="预处理的ZIP基包路径（CI模式）"
    )

    parser.add_argument(
        "--base-apk", type=Path, default=None, help="预处理的APK基包路径（需要解包）"
    )
...
def build_single(
    ...
    # 查找源文件（仅在无基包时需要）
    has_base_package = (
        (base_zip and base_zip.exists())
        or (base_apk and base_apk.exists())
        or (base_apk_dir and base_apk_dir.exists())
    )
...
def build_all(
    pack_type: str, date_param: str, output_dir: Path, source_file: Path = None
) -> int:
    """批量构建所有组合"""
