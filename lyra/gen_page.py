...
class DownloadPageConfig:
    ...
    def get_filename(self, display_name: str, ext: str, is_polyfill: bool = False) -> str:
        """生成文件名"""
        # 移除推荐标记和兼容版标记
        feature_str = display_name.replace('***', '').replace('(推荐)', '').replace('(兼容版)', '').strip()
...
class DownloadPageGenerator:
    ...
    def generate_table_pytablewriter(
        self,
    ...
    def generate(self) -> str:
        ...
        polyfill_besc = [c for c in combinations if c.is_polyfill and "BESC" in c.display_name]
        recommended = [c for c in combinations if c.is_recommended and not c.is_polyfill]
        base = [c for c in combinations if c.display_name == "基础" and not c.is_polyfill]
        others = [c for c in combinations if not c.is_recommended and c.display_name != "基础" and not c.is_polyfill and not (c.is_polyfill and "BESC" in c.display_name)]
