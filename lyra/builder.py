...
from .utils import (
    download_file, extract_zip, create_zip, run_command,
    find_game_file, copy_directory, safe_remove
)
...
class ZipBuilder(PackageBuilder):
    ...
    def build(self, source_file: Path) -> BuildResult:
        ...
        try:
            ...
            logger.info(f"开始构建 包类型: zip, MOD代码: {self.config.mod_code}, 组合: {combination_name}")
        except Exception:
...
class ApkBuilder(PackageBuilder):
    ...
    def _decompile_apk(self, apk_path: Path, apktool_path: Path, dest_dir: Path) -> Path:
        """反编译APK到指定目录"""
        logger.info("反编译APK...")
    ...
    def _sign_apk(self, apk_path: Path, apksign_path: Path) -> Path:
        ...
        run_command([
            "java", "-jar", str(apksign_path),
            "-a", str(apk_path),
            "--ks", str(self.config.keystore_path),
            "--ksAlias", self.config.keystore_alias,
            "--ksKeyPass", self.config.keystore_password,
            "--ksPass", self.config.keystore_password,
            "-o", str(signed_dir)
        ])
    ...
    def build(self, source_file: Path) -> BuildResult:
        ...
        try:
            calculator = CombinationCalculator()
            combination_name = calculator._get_display_name(self.config.mod_code)
            logger.info(f"开始构建 包类型: apk, MOD代码: {self.config.mod_code}, 组合: {combination_name}")
        except Exception:
        ...
        try:
            ...
            safe_remove(tmp_apk)
            safe_remove(self.config.workspace_dir / "signed" / str(self.config.mod_code))
            safe_remove(self._work_dir)
