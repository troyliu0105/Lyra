...
from .utils import (
    ...
    copy_directory, safe_move, safe_remove
)
...
class UCBHandler(BeautifyHandler):
    ...
    def apply(self) -> bool:
        ...
        # 处理问题文件
        img_dir = beautify_dir / "img"
        safe_remove(img_dir / "sex" / "missionary" / "active" / "virginkiller" / "chest.png")
        safe_remove(img_dir / "sex" / "missionary" / "active" / "virginkiller" / "waist.png")
...
class AUHandler(BeautifyHandler):
    ...
    def __init__(self, work_dir: Path, img_path: Path, urls: ResourceURLs, variant: str):
        """
        初始化AU处理器
...
class SideviewBJHandler(BeautifyHandler):
    ...
    def __init__(self, work_dir: Path, img_path: Path, urls: ResourceURLs, assets_dir: Path):
        super().__init__(work_dir, img_path, urls)
